...
int main() {
    ...
    if (!loader.save_stats_csv("results/dataset_stats.csv", st)) {
        std::cerr << "Ошибка: не удалось сохранить файл results/dataset_stats.csv\n";
    }
    if (!loader.save_counts_csv("results/movie_rating_counts.csv",
                                loader.movie_rating_counts(),
                                "movie_id", "ratings_count")) {
        std::cerr << "Ошибка: не удалось сохранить файл results/movie_rating_counts.csv\n";
    }
    if (!loader.save_counts_csv("results/user_rating_counts.csv",
                                loader.user_rating_counts(),
                                "user_id", "ratings_count")) {
        std::cerr << "Ошибка: не удалось сохранить файл results/user_rating_counts.csv\n";
    }

    // === ШАГ 2: train/test + baseline RMSE ===
    std::vector<Rating> train, test;
    const double test_ratio = 0.20;   // 20% в тест
    const uint32_t seed = 42;         // фиксируем, чтобы результаты повторялись
    ...
    std::cout << "Seed: " << seed << ", test_ratio: " << test_ratio << "\n";
    ...
    if (!metrics::save_baseline_csv("results/baseline_rmse.csv",
                                    mu, rmse_mu, rmse_user, cold_cases,
                                    train.size(), test.size())) {
        std::cerr << "Ошибка: не удалось сохранить файл results/baseline_rmse.csv\n";
    } else {
        std::cout << "Baseline-метрики сохранены: results/baseline_rmse.csv\n";
    }

    std::cout << "\nCSV-файлы со статистикой сохранены в папке results/\n";
    return 0;
}
